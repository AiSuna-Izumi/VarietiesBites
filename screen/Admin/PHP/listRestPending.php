...
<?php
        ...
        $sql="SELECT Id,RestaurantName,Email,Location,PhoneNo,password FROM varbite_restregister";
        ...
        while($row = $result->fetch_assoc()) {
         ...
         <tr>
          <form method='post' action='../approveRest.php'>
            ...
            <td ><input type='hidden' name='Loc1' value=".$row["Location"]."><center>".$row["Location"]."</center></td>
            <td ><input type='hidden' name='password' value=".$row["password"]."><center>".$row["password"]."</center></td>
            <td><center><input  value='APPROVE' type='submit'/></form></center></td> 
          </tr>
          </table>
          ";
          }
